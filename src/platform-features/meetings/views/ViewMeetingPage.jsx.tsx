...
const ViewEditMeetingPage: React.FC = () => {
    ...
    // Define interfaces for task and assigned task
    interface Task {
        description: string;
        dueDate: string;
        assignedTo?: string;  // email of the attendee
    }

// Extend the existing state management
    const [tasks, setTasks] = useState<Task[]>([]);
    const [newTask, setNewTask] = useState({ description: '', dueDate: '' });

// Handling task input changes
                            ...
                            <div className="col-span-6 grid grid-cols-8 gap-4 p-2 border rounded-lg border-gray-300">
                                ...
                                <div className="col-span-8">
                                    <button
                                        ...
                                        disabled={newTask.description === '' || newTask.dueDate === '' || selectedContactIndex === ''}
                                    >
                                        Add Task
                                    </button>
                                </div>
                                    ...
                                    <div className="col-span-8 mt-4">
                                        ...
                                        {tasks.map((task, index) => (
                                            <div key={index} className="p-2 border-t border-gray-200">
                                                <p>
                                                    {task.description} - Due: {task.dueDate} - Assigned to:
                                                    {task.assignedTo && contacts[parseInt(task.assignedTo)]
                                                        ? contacts[parseInt(task.assignedTo)].name
                                                        : 'Unassigned'}
                                                </p>                                            </div>
                                        ))}
                                    </div>
