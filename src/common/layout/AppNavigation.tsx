                      ...
                      <DisclosurePanel >
                        {projectTools.map((tool) => (
                          <Link
                            key={tool.name}
                            to={tool.Link}
                            className={`block ml-6 py-2 px-4 text-md font-semibold text-white hover:text-yellow-400 ${isActive(tool.Link) ? 'text-yellow-400 underline' : ''} flex items-center space-x-2`}
                          >
                            <span>{tool.name}</span>
                          </Link>
                        ))}
